...
public class GosControlModuleBaseActivity extends GosBaseActivity {
	...
	// 数据点"电流"对应的标识名
	public static final String KEY_I = "value_I";
	// 数据点"电压"对应的标识名
	protected static final String KEY_V = "value_V";
	// 数据点"功率"对应的标识名
	protected static final String KEY_P = "value_P";
	// 数据点"报警1"对应的标识名
	...
	// 数据点"Fault_V"对应的标识名
	protected static final String KEY_FAULT_V = "Fault_V";
	// 数据点"Fault_I"对应的标识名
	protected static final String KEY_FAULT_I = "Fault_I";
	// 数据点"Fault_P"对应的标识名
	protected static final String KEY_FAULT_P = "Fault_P";
	...
	// 数据点"Fault_V"对应的存储数据
	protected static boolean Fault_V;
	// 数据点"Fault_I"对应的存储数据
	protected static boolean Fault_I;
	// 数据点"Fault_P"对应的存储数据
	protected static boolean Fault_P;
	...
	protected void getDataFromReceiveDataMap(ConcurrentHashMap<String, Object> dataMap) {
		...
		if (dataMap.get("data") != null) {
			...
			for (String dataKey : map.keySet()) {
				...
				if (dataKey.equals(KEY_I)) {
			
					I = (Integer) map.get(dataKey);
				}
				if (dataKey.equals(KEY_V)) {
			
					V = (Integer) map.get(dataKey);
				}
				if (dataKey.equals(KEY_P)) {
			
					P = (Integer) map.get(dataKey);
				}
				...
				if (dataKey.equals(KEY_FAULT_V)) {
					Fault_V = (Boolean) map.get(dataKey);			
				}
				if (dataKey.equals(KEY_FAULT_I)) {
					Fault_I = (Boolean) map.get(dataKey);			
				}
				if (dataKey.equals(KEY_FAULT_P)) {
					Fault_P = (Boolean) map.get(dataKey);			
				}
			}
		}
