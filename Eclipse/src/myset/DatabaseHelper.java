...
public class DatabaseHelper extends SQLiteOpenHelper// 继承SQLiteOpenHelper类
{
	...
	public static DatabaseHelper  getinstance(){
		if(dbh==null){
			dbh=new DatabaseHelper(GosApplication.instance);
		}
		return dbh;
	}
    // 数据库版本号
    ...
    // 数据库名
    private static final String DATABASE_NAME = "CSdata.db";

    // 数据表名，一个数据库中可以有多个表（虽然本例中只建立了一个表）
    public static final String TABLE_NAME = "electricTable";
    public static final String TABLE_NAME_TIMING = "timingTable";
...
//        创建倒计时表
        StringBuffer sBuffer01 = new StringBuffer();

        sBuffer.append("CREATE TABLE [" + TABLE_NAME + "] (");
        sBuffer.append("[_id] INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, ");
        sBuffer.append("[name] TEXT,");
        sBuffer.append("[timeStart] TEXT,");
        sBuffer.append("[timeOver] TEXT,");
        sBuffer.append("[timeUse] TEXT,");
        sBuffer.append("[electricQuantity] TEXT)");

        sBuffer01.append("CREATE TABLE [" + TABLE_NAME_TIMING + "] (");
        sBuffer01.append("[_id] INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, ");
        sBuffer01.append("[OnOff] TEXT,");
        sBuffer01.append("[time] TEXT,");
        sBuffer01.append("[repetition] TEXT,");
        sBuffer01.append("[ifWork] TEXT)");
        // 执行创建表的SQL语句
        db.execSQL(sBuffer.toString());
        db.execSQL(sBuffer01.toString());
